...
| `jira` | Context source | `driver`, `base_url`, `email`, one of `api_token` or `api_token_ref`, `projects`, `labels` | `since` |
...
| `api_token` / `api_token_ref` | Literal Atlassian API token or reference. Prefer the `_ref`. |
| `projects` | Jira project keys to ingest (e.g. `["DOCS", "ENG"]`). At least one required. |
| `labels` | Label allowlist — only issues with at least one of these labels are fetched. At least one required. |
| `since` | ISO date (`YYYY-MM-DD`). Lower bound for the first full fetch. Subsequent runs derive the floor from the `updated` timestamp of existing staged issues. |
...
| `sigma` | Sigma data model specs and workbook summaries. |
| `notion` | Notion pages and databases for wiki context. |
| `jira` | Jira issues filtered by project and label for wiki context. |
