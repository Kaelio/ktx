...
- **A missing structured key means the identifier lives in the prose — parse it into the join.** When the table the question names has no id/name column to join on (only free-text/narrative columns), the entity identifier is embedded in that prose, and extracting it *is* the join path: parse the name/key into a CTE (see *Extract the number from prose* for the sample-the-phrasings, `COALESCE`-the-patterns habit — it applies to a key just as to a metric) and join downstream tables on the parsed value, rather than concluding the tables cannot be linked. Confirm the parsed key matches the target column's real form (case, separators, owner/name shape) on a sample before trusting the join. *Why:* a table with no key column still participates in joins through its prose — treating "no key column" as "not joinable" silently drops that whole table's rows from the answer's universe.
...
- **Extract the number from prose by covering every phrasing, not one pattern.** When the value to rank or aggregate by lives inside a free-text/narrative column rather than its own numeric column, first **sample the distinct phrasings** the same fact takes — generated/scraped narratives routinely state one fact several ways (`"30,522 forks"` number-before *and* `"forks count of 7201"` number-after) — then extract with a `COALESCE` over one regex alternative **per phrasing** (get each engine's regex/substring-extract idiom from `sql_dialect_notes`), keeping each token *with its separators* so a later *Strip, scale, and cast* can clean it. A single pattern silently parses the forms it matches and drops the rest to `NULL`, so entities described the other way vanish from the ranking and a top-N loses a true leader with no error; a `,`/`_`/space thousands-separator dropped at extraction time (`"30,522"`→`30`) is the same failure in miniature. *Why:* the number is not isolated until extraction lifts it out, and one pattern only isolates one phrasing.
