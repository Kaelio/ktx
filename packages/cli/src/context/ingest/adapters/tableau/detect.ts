...
export async function detectTableauStagedDir(stagedDir: string): Promise<boolean> {
  try {
    await stat(join(stagedDir, STAGED_FILES.manifest));
  } catch {
    return false;
  }
  for (const subdir of [STAGED_FILES.datasourcesDir, STAGED_FILES.workbooksDir]) {
    let entries: string[];
    try {
