...
- **Extract from text:** to rank/aggregate by a number buried in a narrative column, pull the token *with its separators* first — `extract(txt, '[0-9][0-9.,_ ]*')` — then apply the strip/scale/cast and **Safe cast** above. The class keeps `,`/`_`/space so `'30,522 forks'` yields `'30,522'`, not `'30'`; extracting a bare `[0-9]+` would silently truncate at the first separator.
