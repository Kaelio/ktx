...
type RedshiftTableNameRef = Pick<KtxTableRef, 'name'> & Partial<Pick<KtxTableRef, 'catalog' | 'db'>>;

/** @internal */
export class KtxRedshiftDialect implements KtxDialect {
  ...
  formatTableName(table: RedshiftTableNameRef): string {
    return formatDialectTableName(table, this.quoteIdentifier.bind(this), 'ansi');
  }

  formatDisplayRef(table: RedshiftTableNameRef): string {
