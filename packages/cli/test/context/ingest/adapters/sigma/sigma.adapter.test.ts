...
describe('SigmaSourceAdapter.listTargetConnectionIds', () => {
  ...
  async function writeManifest(sigmaConnectionId: string) {
    await writeFile(
      join(stagedDir, 'sigma-manifest.json'),
      JSON.stringify({ sigmaConnectionId, fetchedAt: '2026-01-01T00:00:00Z', dataModelCount: 0, workbookCount: 0 }),
      'utf-8',
    );
  }
  ...
  it('returns mapped warehouse connection IDs when mappings are present', async () => {
    ...
    await writeManifest('sigma-prod');
  ...
  it('falls back to the Sigma connection ID from the manifest when connectionMappings is empty', async () => {
    await writeProjectionConfig({});
    await writeManifest('sigma-prod');
    const adapter = new SigmaSourceAdapter({ clientFactory: makeFactory() });
    const ids = await adapter.listTargetConnectionIds(stagedDir);
    expect(ids).toEqual(['sigma-prod']);
  });

  it('falls back to the manifest when the projection config file is missing', async () => {
    await writeManifest('sigma-prod');
    const adapter = new SigmaSourceAdapter({ clientFactory: makeFactory() });
    const ids = await adapter.listTargetConnectionIds(stagedDir);
    expect(ids).toEqual(['sigma-prod']);
  ...
  it('falls back to the manifest when the projection config is malformed', async () => {
